...
function openModal(k, p, free) {
    ...
    // Reset inputs for clean checkout
...
function calculateGrandTotal() {
    ...
    let perRoom = pkg.p;
    // FIX: Lanyard logic RM5 per extra unit beyond free quota
    let extraL = Math.max(0, lQty - pkg.free);
    perRoom += (extraL * 5); 
    
    if(document.getElementById('w-check').checked) { perRoom += 100; }

    let total = perRoom * rQty;
    if(isInst) { total = total / dur; }
...
function generateInvoice() {
    ...
    document.getElementById('final-label').innerText = isInst ? `Monthly Payment (${dur}m)` : 'Grand Total';
    ...
    if(extraL > 0) {
        rows += `<tr><td class="py-4 px-4 border-b border-slate-50">Custom Lanyard Add-on</td><td class="text-center border-b border-slate-50">${extraL * rQty}</td><td class="text-right px-4 border-b border-slate-50">RM ${(extraL * 5 * rQty).toFixed(2)}</td></tr>`;
    }
    if(document.getElementById('w-check').checked) {
        rows += `<tr><td class="py-4 px-4 border-b border-slate-50">Extended Warranty (2-Year)</td><td class="text-center border-b border-slate-50">${rQty}</td><td class="text-right px-4 border-b border-slate-50">RM ${(100 * rQty).toFixed(2)}</td></tr>`;
    }
